...
class Indexer(object):
    def __init__(self, db, voc):
        """ Initialize with the name of the database database
    ...
    def add_to_index(self, imname, descr):
        """ Take an image with feature descriptors,
            project on vocabulary and ad to database. """
        ...
        for i in range(nbr_words):
            ...
            self.con.execute(
                "insert into imwords(imid, wordid, vocname) \
        ...
        self.con.execute(
            "insert into imhistograms(imid, histogram, vocname) \
                values (?, ?, ?)", (imid, pickle.dumps(imwords), self.voc.name)
        )

    def is_indexed(self, imname):
        """ Returns True id imname has been indexed. """

        im = self.con.execute("select rowid from imlist where filename\
            ='%s'" % imname).fetchone()
        return im != None
    
    def get_id(self, imname):
        """ Get an entry id and add id not present. """
...
class Searcher(object):
    ...
    def candidates_from_word(self, imword):
        ...
        # print('candidate from word ', im_ids)
    ...
    def get_imhistogram(self, imname):
        """ Return the word histogram for a image. """
    ...
    def query(self, imname):
        ...
        # return a sorted list of distance and database ids
        matchscores.sort()
        return matchscores
...
def plot_results(src, res):
    ...
    for i in range(nbr_results):
        ...
        imshow(np.array(Image.open(imname)))
        axis('off')
    show()
