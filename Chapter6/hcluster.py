...
class ClusterNode(object):
    def __init__(self, vec, left, right, distance=0.0, count=1) -> None:
        super().__init__()
        self.left = left 
        self.right = right
        self.vec = vec
        self.distance = distance
        self.count = count # used for weighted average
    ...
    def get_depth(self):
        """ Depth is max of each child to root distance. """
...
def hcluster(features, distfcn=L2dist):
    """ Cluster the rows of feature using
    ...
    while len(node) > 1: # If node is root, stop create tree
        closest = float('Inf') # Infinitive
...
def draw_dendrogram(node, imlist, filename='clusters.jpg'):
    ...
    # scale factor for disrances to fit image width
